...
def get_basis_vectors(lattice, a, b, c, alpha, beta, gamma):
    match lattice:
        case "cP":
            return (np.array([[a, 0, 0], [0, a, 0], [0, 0, a]], dtype=np.float64), np.array([[1,0,0], [0,1,0], [1,1,0], [1,1,1]], dtype=np.float64))
        case "cI":
            return ((a/2)*np.array([[-1, 1, 1], [1, -1, 1], [1, 1, -1]], dtype=np.float64), np.array([[1,0,0], [0,1,0], [1,1,0], [1,1,1]], dtype=np.float64))
        case "cF":
            return ((a/2)*np.array([[0, 1, 1], [1, 0, 1], [1, 1, 0]], dtype=np.float64), np.array([[1,0,0], [0,1,0], [1,1,0], [1,1,1]], dtype=np.float64))
        case "tP":
...
def rot_matrix(miller_indices, rot_axis):
    ...
    u = miller_indices.copy().astype(np.float64)
    u /= np.linalg.norm(u)
    v = rot_axis.copy().astype(np.float64)
    v /= np.linalg.norm(v)
...
def calculate_in_plane_vectors(lat, a, b, c, alpha, beta, gamma, prefix):
    ...
    for i, _ in enumerate(symmetry_directions):
        ...
        z0_plane = select_atoms_by_plane(rotated_lattice, z=0.0)
        ...
        with open(dat_file, "w") as f:
            f.write("Index    x    y    z\n")
            for i, vec in enumerate(shortest_vectors):
...
rotated_points = rotate_lattice(lattice_points, target_ax, original_ax)

# Select all atoms that lie in the plane z = 0.
...
new_old_vectors = sort_by_distance(rotate_lattice(shortest_vectors, original_ax, target_ax), 0.0, 0.0)
